...
from app.api.deps import (
    ...
    UserRepo, RoleRepo, PermissionRepo, BusinessLineRepo,
)
...
async def validate_role_assignment(
    role_repo, business_line_repo,
    current_user, role_ids: List[int]
) -> tuple[bool, str]:
    """验证用户是否有权限分配指定角色
...
async def list_users(
    ...
    for user in users:
        ...
        user_perms = await permission_repo.get_user_permissions(user.id)
        permissions = [p.permission_code for p in user_perms]

        # 获取用户所属业务线
        user_business_lines = await business_line_repo.get_user_business_lines(user.id)
        for ubl in user_business_lines:
            bl = await business_line_repo.get_business_line_by_id(ubl.business_line_id)
            if bl:
                user_bls.append(
                    BusinessLineInfo(
                        id=bl.id,
        ...
        # 获取用户管理的业务线
        user_managed_business_lines = await business_line_repo.get_user_managed_business_lines(
            user.id
        )
...
async def get_user(
    ...
    # 获取用户管理的业务线
    user_managed_business_lines = await business_line_repo.get_user_managed_business_lines(user.id)
    for ubl in user_managed_business_lines:
        bl = await business_line_repo.get_business_line_by_id(ubl.business_line_id)
        if bl:
            user_managed_bls.append(
...
async def create_user(
    ...
    if request.role_ids:
        is_valid, error_msg = await validate_role_assignment(
            role_repo, business_line_repo, current_user, request.role_ids
        )
        if not is_valid:
    ...
    # 分配角色
    if request.role_ids:
        await role_repo.set_user_roles(user.id, request.role_ids)

    # 分配业务线：至少分配一个业务线，默认使用"其他"业务线
    bl_ids = request.business_line_ids if request.business_line_ids else []
    if not bl_ids:
        other_bl = await business_line_repo.get_business_line_by_code("OTHER")
        if other_bl:
            bl_ids = [other_bl.id]

    for bl_id in bl_ids:
        await business_line_repo.add_user_to_business_line(user.id, bl_id)
...
async def update_user(
    ...
    # 更新角色
    if request.role_ids is not None:
        await role_repo.set_user_roles(user_id, request.role_ids)
...
async def reset_password(
    ...
    await user_repo.update_user(user_id, password_hash=hash_password(new_password))
