...
class SessionManagementService:
    ...
    async def start(self):
        ...
        logger.info(
            f"Session management service started, timeout: {settings.session_timeout_minutes} minutes"
        )
    ...
    async def create_session(self, gateway_id: str, api_key: str) -> SessionConfig:
        ...
        session = SessionConfig(
            session_id=session_id, gateway_id=gateway_id, api_key=api_key
        )

        self._sessions[session_id] = session

        logger.info(
            f"Created session {session_id} for gateway {gateway_id}, active sessions: {len(self._sessions)}"
        )
    ...
    async def remove_session(self, session_id: str):
        ...
        if session:
            ...
            logger.info(
                f"Removed session {session_id}, remaining sessions: {len(self._sessions)}"
            )
    ...
    async def _cleanup_expired_sessions(self):
        ...
        while self._running:
            try:
                ...
                if expired:
                    logger.info(
                        f"Cleaned up {len(expired)} expired sessions, remaining: {len(self._sessions)}"
                    )
